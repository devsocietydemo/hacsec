...
Unfortunately, on some laptops port `80` might already be in use by another application, and we will have to use another one. To verify it, run the following command in Windows Command Prompt: `netstat -an | grep LISTEN | grep ':80 '`. You expect no entries in response, like that:
----
C:\Users\dawid.buchwald>netstat -an | grep LISTEN | grep ':80 '
...
----
C:\Users\dawid.buchwald>netstat -an | grep LISTEN | grep ':81 '   <1>
  TCP    0.0.0.0:81            0.0.0.0:0              LISTENING
  TCP    [::]:81               [::]:0                 LISTENING

C:\Users\dawid.buchwald>netstat -an | grep LISTEN | grep ':8081 ' <2>
  TCP    0.0.0.0:8081          0.0.0.0:0              LISTENING
  TCP    [::]:8081             [::]:0                 LISTENING

C:\Users\dawid.buchwald>netstat -an | grep LISTEN | grep ':8080 ' <3>
...
Run the following command in Windows Command Prompt to get the files (Please note: this is single line command!) `cp -r \\morgana.ams.com\Public\Hackathons\Security\CompatibilityTool .`:
----
C:\Users\dawid.buchwald>cp -r \\morgana.ams.com\Public\Hackathons\Security\CompatibilityTool .
